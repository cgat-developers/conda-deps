...
do
    ...
    log " with: $env_f"
    python3 conda_deps/conda_deps.py --debug $f
    diff <(python3 conda_deps/conda_deps.py $f) <(cat $env_f)
    if [[ "$?" -eq "0" ]] ; then
...
python3 conda_deps/conda_deps.py --debug $ALL
diff <(python3 conda_deps.py $ALL) <(cat tests/all.yml)
if [[ "$?" -eq "0" ]] ; then
